...
- Zarf (`sh:`, `ef:Package`) ve uygulama/sistem yanıtı (`apr:ApplicationResponse`) kurallarının tamamına Türkçe açıklama (67 kural); toplam kapsam %77,1 (229/297). Açıklamasız kalan yalnız GİB kullanıcı hesabı belgeleri (`hr:ProcessUserAccount`) ve 4 bağlamsız kural.
...
- `ozet` / `belge_ozeti`: e-İrsaliye ve irsaliye yanıtında `satirSayisi` doluyken `satirlar` boş dönüyordu (yalnız `InvoiceLine` okunuyordu); artık `DespatchLine`/`DeliveredQuantity` ve `ReceiptLine`/`ReceivedQuantity` de okunuyor.
